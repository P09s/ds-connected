...
function Enemy({ hp, maxHp, hit, dead, name, swordConnected }) {
  const barPct = Math.max(0, (hp / maxHp) * 100)
  const barColor = barPct > 60 ? '#ff4444' : barPct > 30 ? '#ff8800' : '#ff0000'
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px' }}>
      <div style={{ fontFamily: "'Cinzel', serif", fontSize: '0.7rem', letterSpacing: '0.4em', color: 'rgba(255,100,100,0.7)', textTransform: 'uppercase' }}>
        {name}
      </div>
      <div style={{ width: '300px', maxWidth: '60vw' }}>
        ...
        <div style={{ width: '100%', height: '6px', background: 'rgba(255,0,0,0.15)', borderRadius: '3px', overflow: 'hidden', border: '1px solid rgba(255,50,50,0.2)' }}>
          {/* FIXED: Removed backslashes escaping the template literals */}
          <div style={{ width: `${barPct}%`, height: '100%', background: barColor, boxShadow: `0 0 8px ${barColor}`, transition: 'width 0.3s ease-out', borderRadius: '3px' }}/>
        </div>
      </div>
      <div className={hit ? 'enemy-hit' : dead ? 'enemy-death' : 'enemy-float'} style={{ position: 'relative', opacity: swordConnected ? 1 : 0.35, transition: 'opacity 0.5s' }}>
        ...
        {!swordConnected && !dead && (
          <div className="breathe" style={{ position: 'absolute', bottom: '-28px', left: '50%', transform: 'translateX(-50%)', fontSize: '0.55rem', letterSpacing: '0.3em', color: 'rgba(255,100,100,0.4)', whiteSpace: 'nowrap' }}>
            WAITING FOR SWORD...
          </div>
        )}
        {dead && (
          <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', fontFamily: "'Shippori Mincho', serif", fontSize: '2rem', fontWeight: 800, color: '#ff4444', letterSpacing: '0.3em', whiteSpace: 'nowrap', textShadow: '0 0 20px rgba(255,0,0,0.8)' }}>
            滅 SLAIN
          </div>
        )}
      </div>
...
export default function LaptopPage() {
  ...
  const [swordConnected, setSwordConnected] = useState(false)
  ...
  const handleSlashRef = useRef(null)
  handleSlashRef.current = (data) => {
    ...
    if (canvasRef.current) spawnDamageNumber(canvasRef.current, x, y, dmg)
    
    triggerShake()
    triggerEnemyHit()
    
    if (newHp <= 0 && !enemyDeadRef.current) {
  ...
  return (
    <div ref={screenRef} className={shaking ? 'screen-shake' : ''} style={{ position: 'fixed', inset: 0, background: 'radial-gradient(ellipse at 50% 30%, #0d1a2e 0%, #020b18 70%)', overflow: 'hidden' }}>
      <svg className="fixed inset-0 w-full h-full opacity-[0.06] pointer-events-none">
        ...
        {Array.from({ length: 12 }, (_, i) => (
          <path key={i} d={`M -200 ${40 + i * 80} Q ${window.innerWidth*0.4} ${20+i*80} ${window.innerWidth*0.7} ${55+i*80} T ${window.innerWidth+200} ${40+i*80}`} stroke="#7DF9FF" strokeWidth="1" fill="none"/>
        ))}
      </svg>
      <div ref={canvasRef} className="fixed inset-0 pointer-events-none" style={{ zIndex: 40 }}/>
      <div style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 90, display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 24px', background: 'linear-gradient(180deg, rgba(2,11,24,0.9) 0%, transparent 100%)' }}>
        ...
        <div style={{ fontSize: '0.55rem', letterSpacing: '0.3em', color: 'rgba(125,249,255,0.4)' }}>{slashCount} strikes</div>
      </div>

      {!swordConnected && (
        <div style={{ position: 'fixed', bottom: '80px', left: '50%', transform: 'translateX(-50%)', zIndex: 30, textAlign: 'center' }}>
          <div style={{ padding: '12px 28px', border: '1px solid rgba(125,249,255,0.15)', borderRadius: '4px', background: 'rgba(125,249,255,0.02)' }}>
            <div style={{ fontSize: '0.45rem', letterSpacing: '0.4em', color: 'rgba(125,249,255,0.25)', marginBottom: '6px' }}>ROOM ID — ENTER ON PHONE</div>
            <div style={{ fontFamily: "'Cinzel', serif", fontSize: '1.4rem', fontWeight: 700, color: 'rgba(125,249,255,0.6)', letterSpacing: '0.3em' }}>{roomId}</div>
          </div>
        </div>
      )}

      <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', zIndex: 20, textAlign: 'center' }}>
        <Enemy hp={hp} maxHp={MAX_HP} hit={enemyHit} dead={enemyDead} name={enemyName} swordConnected={swordConnected}/>
        {enemyDead && <div style={{ marginTop: '16px', fontSize: '0.6rem', letterSpacing: '0.4em', color: 'rgba(255,100,100,0.5)' }}>Respawning...</div>}
      </div>
    </div>
  )
}